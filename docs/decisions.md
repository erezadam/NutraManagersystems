...
| 2026-02-20 | לממש שכבת Data Access גנרית (`EntityService`) על `fetch` | סטנדרטיזציה לקריאות Base44 וצמצום שכפול | לממש שירות נפרד מלא לכל ישות | פישוט תחזוקה ובדיקות חוזה API |
...
| 2026-02-22 | לאמץ Firebase כבסיס נתונים ויעד פריסה בענן (Firestore + Hosting + Storage) עם fallback זמני ל-Base44 | דרישת מוצר לפריסה בענן בשליטה מלאה ועצמאות ספק | מעבר חד ללא fallback | מאפשר מעבר מדורג ללא השבתה תוך שמירה על תאימות זמנית |
